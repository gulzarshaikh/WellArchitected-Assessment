...
# Base configuration
$md = Get-Content -Path ./input/security-assesment.md
$outfilename = "./output/app-security.json"
$pillar = "security"
...
$md | % {
    ...
    elseif($_.ToString() -match "^\s*[*]\s*")
    {
        ...
        $currentItem = [ordered]@{
            ...
            title = strip -text $_;
            recommendation = "";
            id = "$($currentCategory.ToLower().Replace("&", """").Replace(" ", "_"))_$(Get-Random -Maximum 9999)"
        }
    }
